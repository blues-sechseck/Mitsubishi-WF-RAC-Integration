...
| Indoor Coil Temperature *(diagnostic, only with Service Data on)* | °C | Indoor heat-exchanger temperature (MHI's THI-R1). Per indoor unit, not shared. In cooling it drops as the coil gets cold and rises back to room temperature once the compressor stops - the clearest signal there is for what the unit is actually doing. Same "Service Data" requirement as above. |
