...
| Indoor Coil Sensor (raw), Indoor Coil Outlet Sensor (raw) *(diagnostic, disabled by default, only with Service Data on)* | *(none)* | The bytes the two temperatures above are converted from. The conversion comes from the sensor's own characteristic and was measured against a thermometer on the coil from 23 to 46 °C; above roughly byte 170 it is extrapolation. Off by default - enable them if you want to check the conversion on your own unit. |
