...
| Outdoor Coil Sensor (raw) *(diagnostic, disabled by default, only with Service Data on, shared on multi-split)* | *(none)* | Raw byte from the outdoor heat-exchanger sensor (MHI's THO-R1), outdoor-unit-level. Deliberately unconverted: it is the same kind of sensor as the indoor coil but sits behind a different series resistor, so the indoor conversion does not apply to it, and its own has not been measured against a thermometer yet. |
